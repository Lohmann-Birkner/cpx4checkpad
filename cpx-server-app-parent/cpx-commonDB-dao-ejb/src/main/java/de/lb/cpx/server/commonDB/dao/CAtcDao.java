...
public class CAtcDao extends AbstractCommonDao<CAtc> {
    ...
    public boolean catalogExists(final String pChecksum) {
        return super.catalogExists("C_ATC", pChecksum);
    }
    ...
    public boolean catalogExists(final String pChecksum, final CountryEn pCountryEn) {
        return super.catalogExists("C_ATC", pChecksum, pCountryEn);
    }
    public int getEntryCounter(final CountryEn pCountryEn) {
        String query = "SELECT COUNT(*) FROM C_ATC "
    ...
    public int dropEntries(/* final int pYear, */final CountryEn pCountryEn) {
        int deleteCount = 0;
        String query = "DELETE FROM C_ATC "
                + " WHERE COUNTRY_EN = :country ";
    ...
    public List<CAtc> getEntries(/* final int pYear, */final String pCountryEn) {
        List<CAtc> list = null;
        Query query = getEntityManager().createQuery("from " + CAtc.class.getSimpleName() + " a where a.countryEn = :country order by a.atcCode1");
