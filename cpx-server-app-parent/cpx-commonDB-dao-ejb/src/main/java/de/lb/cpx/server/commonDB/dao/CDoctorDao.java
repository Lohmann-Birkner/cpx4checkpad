...
public class CDoctorDao extends AbstractCommonDao<CDoctor> {
    ...
    public boolean catalogExists(final String pChecksum) {
        return super.catalogExists("C_DOCTOR", pChecksum);
    }
    ...
    public boolean catalogExists(final String pChecksum, final CountryEn pCountryEn) {
        return super.catalogExists("C_DOCTOR", pChecksum, pCountryEn);
    }
    
    public int getEntryCounter(final CountryEn pCountryEn) {
        String query = "SELECT COUNT(*) FROM C_DOCTOR "
    ...
    public int dropEntries(/* final int pYear, */final CountryEn pCountryEn) {
        int deleteCount = 0;
        String query = "DELETE FROM C_DOCTOR "
                + " WHERE COUNTRY_EN = :country ";
    ...
    public List<CDoctor> getEntries(/* final int pYear, */final String pCountryEn) {
        List<CDoctor> list = null;
        Query query = getEntityManager().createQuery("from " + CDoctor.class.getSimpleName() + " a where a.countryEn = :country order by a.docIdent");
