...
/**
 * InputMap is a class that is set on a given {@link Node}. When the Node
 * receives an input event from the system, it passes this event in to the
 * InputMap where the InputMap can check all installed
 * {@link InputMap.Mapping mappings} to see if there is any suitable mapping,
...
//    }
    // --- interceptor
    /**
     * The role of the interceptor is to block the InputMap on which it is set
     ...
     * as this allows for easy addition of mappings that are state-specific. For
     * example, if a Node can be in two different states, and the input mappings
     * are different for each, then it makes sense to have one root (and empty)
     * InputMap, with two children input maps, where each is populated with the
     * specific input mappings for one of the two states. To prevent the wrong
     * input map from being considered, it is simply a matter of setting an
     * appropriate {@link #interceptorProperty() interceptor} on each map, so
     * that they are only considered in one of the two states.
     *
     * @return list of node inputmaps
     */
...
//                System.out.println("Removed event handler for type " + et);
    ...
    public static abstract class Mapping<T extends Event> {
         ...
         * the match. This allows the InputMap to determine which mapping is
    ...
    public static class KeyMapping extends Mapping<KeyEvent> {
        ...
        /**
         * Creates a new KeyMapping instance that will fire when the given
         * {@link KeyBinding} is entered into the application by the user, and
     ...
     * at a {@link InputMap#interceptorProperty() input map} level or a
     * {@link Mapping#interceptorProperty() mapping} level.
     *
    ...
    public static class MouseMappingInterceptor implements Predicate<Event> {
         ...
         * should be blocked (typically one of null null null null null null
         * null null         {@link MouseEvent#MOUSE_PRESSED},
