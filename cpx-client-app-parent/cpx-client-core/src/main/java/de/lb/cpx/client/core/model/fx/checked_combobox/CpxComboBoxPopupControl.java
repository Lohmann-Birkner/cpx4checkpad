 ...
 * @since 9
 */
public abstract class CpxComboBoxPopupControl<T> extends ComboBoxBaseSkin<T> {

    private static final Logger LOG = Logger.getLogger(CpxComboBoxPopupControl.class.getName());
...
//    private T recentValue;
     ...
     */
    public CpxComboBoxPopupControl(ComboBoxBase<T> control) {
        super(control);
            ...
            if (c != null) {
                ...
                if (oldValue == null && (text == null || text.isEmpty())) {
                    ...
                    try {
                        ...
                        LOG.log(Level.FINEST, "Most likely a parsing error, such as DateTimeParseException", ex);
                    }
                }
...
//    }
    ...
    private void createPopup() {
        popup = new PopupControl() {
            ...
            public Styleable getStyleableParent() {
                return CpxComboBoxPopupControl.this.getSkinnable();
            }
        ...
        popup.setOnAutoHide(e -> getBehavior2().onAutoHide(popup));
    ...
    protected abstract ComboBoxBaseBehavior<T> getBehavior2();
    ...
    void reconfigurePopup() {
        ...
        if (p.getX() > -1) {
            popup.setAnchorX(p.getX());
        }
        if (p.getY() > -1) {
            popup.setAnchorY(p.getY());
        }
        if (minWidth > -1) {
        ...
        if (Double.doubleToRawLongBits(newWidth) != Double.doubleToRawLongBits(currentWidth)
                || Double.doubleToRawLongBits(newHeight) != Double.doubleToRawLongBits(currentHeight)) {
    ...
    public void setPopupNeedsReconfiguring(final boolean pValue) {
        popupNeedsReconfiguring = pValue;
    }
