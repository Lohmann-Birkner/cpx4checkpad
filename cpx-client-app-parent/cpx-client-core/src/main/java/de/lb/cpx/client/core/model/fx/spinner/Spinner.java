 ...
 * {@link SpinnerValueFactory#wrapAroundProperty() wrapping} and simpler
 * specification of 'infinite' data models (the
 ...
 * {@link SpinnerValueFactory SpinnerValueFactory}. The value factory can be
 * specified as a constructor argument and changed with the
 ...
 * {@link SpinnerValueFactory#valueProperty() value property} of the value
 * factory. If the user has changed the value displayed in the editor it is
 * possible for the Spinner {@link #valueProperty() value} to differ from that
 * of the editor. To make sure the model has the same value as the editor, the
 * user must commit the edit using the Enter key.
 *
 * @see SpinnerValueFactory
 * @param <T> The type of all values that can be iterated through in the
 * Spinner. Common types include Integer and String.
 * @since JavaFX 8u40
 */
...
public class Spinner<T> extends Control {
    ...
    // --- valueFactory
    /**
     * The value factory is the model behind the JavaFX Spinner control -
     * without a value factory installed a Spinner is unusable. It is the role
     * of the value factory to handle almost all aspects of the Spinner,
     * including:
     *
     * <ul>
     * <li>Representing the current state of the
     * {@link SpinnerValueFactory#valueProperty() value},</li>
     * <li>{@link SpinnerValueFactory#increment(int) Incrementing} and
     * {@link SpinnerValueFactory#decrement(int) decrementing} the value, with
     * one or more steps per call,</li>
     ...
     * <li>Converting
     * {@link SpinnerValueFactory#converterProperty() objects to user-readable strings}
    ...
    // --- editable
    /**
     * The editable property is used to specify whether user input is able to be
     * typed into the Spinner {@link #editorProperty() editor}. If editable is
     * true, user input will be received once the user types and presses the
     * Enter key. At this point the input is passed to the SpinnerValueFactory {@link SpinnerValueFactory#converterProperty() converter}
     * {@link javafx.util.StringConverter#fromString(String)} method. The
     * returned value from this call (of type T) is then sent to the
     * {@link SpinnerValueFactory#setValue(Object)} method. If the value is
     * valid, it will remain as the value. If it is invalid, the value factory
     ...
     * {@link #valueFactoryProperty() value factory} set to be an instance of
     * {@link SpinnerValueFactory.IntegerSpinnerValueFactory}. Note that if this
     * constructor is called, the only valid generic type for the Spinner
     ...
     * Creates a Spinner instance with the
     * {@link #valueFactoryProperty() value factory} set to be an instance of
     * {@link SpinnerValueFactory.DoubleSpinnerValueFactory}. Note that if this
     * constructor is called, the only valid generic type for the Spinner
     ...
     * Creates a Spinner instance with the
     * {@link #valueFactoryProperty() value factory} set to be an instance of
     * {@link SpinnerValueFactory.LocalDateSpinnerValueFactory}. Note that if
     * this constructor is called, the only valid generic type for the Spinner
     * instance is LocalDate, i.e. Spinner&lt;LocalDate&gt;.
     *
     ...
     * value will be used.
     * @param amountToStepBy The amount to increment or decrement by, per step.
     * @param temporalUnit The size of each step (e.g. day, week, month, year,
     * etc).
     */
    Spinner(@NamedArg("min") LocalDate min,
            @NamedArg("max") LocalDate max,
     ...
     * Creates a Spinner instance with the
     * {@link #valueFactoryProperty() value factory} set to be an instance of
     * {@link SpinnerValueFactory.LocalTimeSpinnerValueFactory}. Note that if
     * this constructor is called, the only valid generic type for the Spinner
     * instance is LocalTime, i.e. Spinner&lt;LocalTime&gt;.
     *
     ...
     * value will be used.
     * @param amountToStepBy The amount to increment or decrement by, per step.
     * @param temporalUnit The size of each step (e.g. hour, minute, second,
     * etc).
     */
    Spinner(@NamedArg("min") LocalTime min,
            @NamedArg("max") LocalTime max,
     ...
     * Creates a Spinner instance with the
     * {@link #valueFactoryProperty() value factory} set to be an instance of
     * {@link SpinnerValueFactory.ListSpinnerValueFactory}. The Spinner
     * {@link #valueProperty() value property} will be set to the first element
     * of the list, if an element exists, or null otherwise.
     *
     ...
     *
     * @param valueFactory The {@link #valueFactoryProperty() value factory} to
     * use.
     */
    ...
    * the Spinner wrapAround property is true.
    ...
    static BigDecimal wrapValue(BigDecimal value, BigDecimal min, BigDecimal max) {
        if (Double.doubleToRawLongBits(max.doubleValue()) == Double.doubleToRawLongBits(0.0d)) {
            throw new RuntimeException();
        }
     ...
     *
     * @throws IllegalStateException if the value factory returned by calling
     * {@link #getValueFactory()} is null.
     */
     ...
     * {@link SpinnerValueFactory#increment(int)} method and forwarding the
     * steps argument to it. If the value factory is null, an
     * IllegalStateException is thrown.
     *
     * @param steps The number of increments that should be performed on the
     * value.
     * @throws IllegalStateException if the value factory returned by calling
     * {@link #getValueFactory()} is null.
     */
     ...
     * Attempts to decrement the {@link #valueFactoryProperty() value factory}
     * by the given number of steps, by calling the
     * {@link SpinnerValueFactory#decrement(int)} method and forwarding the
     * steps argument to it. If the value factory is null, an
     * IllegalStateException is thrown.
     *
     * @param steps The number of decrements that should be performed on the
     * value.
     * @throws IllegalStateException if the value factory returned by calling
     * {@link #getValueFactory()} is null.
     */
    ...
    /**
     * If the Spinner is {@link #editableProperty() editable}, calling this
     * method will attempt to commit the current text and convert it to a
     * {@link #valueProperty() value}.
     *
     * @since 9
     */
    ...
    /**
     * If the Spinner is {@link #editableProperty() editable}, calling this
     * method will attempt to replace the editor text with the last committed
     * {@link #valueProperty() value}.
     *
     * @since 9
     */
     ...
     * will be unbound from the old value factory and bound to the new one.<p>
     * If developers wish to modify the value property, they may do so with code
    ...
    // --- prompt text
    /**
     * The prompt text to display in the {@code Spinner}, or {@code null} if no
    ...
    /**
     * The duration that the mouse has to be pressed on an arrow button before
     * the next value steps. Successive step duration is set using
