 ...
 * @author wilde
 */
public abstract class BasicTopicHandler {

    private static final Logger LOG = Logger.getLogger(BasicTopicHandler.class.getSimpleName());
     ...
     */
    public BasicTopicHandler() {
        try {
            init("java:/jms/topic/CpxJmsStatusBroadcast", "java:/jms/topic/CpxJmsStatusBroadcast");
     ...
     */
    public BasicTopicHandler(String clientId) {
     ...
     */
    public BasicTopicHandler(String responseQueue, String requestQueue) {
        try {
     ...
     * @param clientId to identify the client
     */
    public BasicTopicHandler(String responseQueue, String requestQueue, String clientId) {
...
//        props.put(Context.INITIAL_CONTEXT_FACTORY, "org.wildfly.naming.client.WildFlyInitialContextFactory");
    ...
    public String getFilter() {
        return "(PersistenceUnit IS NULL) OR (PersistenceUnit = '') OR (PersistenceUnit = '" + de.lb.cpx.client.core.config.Session.instance().getCpxPersistenceUnit() + "')";
    }
...
//            requestTopic = (Topic) ctx.lookup(requestChannel);
//        }
        if (!responseChannel.isEmpty()) {
            responseTopic = (Topic) ctx.lookup(responseChannel);
        }
...
//            currentId="1";
            //consumer = jmsSession.createConsumer(responseTopic, "(Database IS NULL) OR (Database = '') OR (Database = '" + de.lb.cpx.client.core.config.Session.instance().getCpxDatabase() + "')"); //, "ClientId = '" + currentId + "'");
            final String filter = getFilter();
            consumer = jmsSession.createConsumer(responseTopic, filter); //, "ClientId = '" + currentId + "'");
     ...
     */
    public final void setOnMessageListener(MessageListener listener) throws JMSException {
        if (consumer != null) {
            consumer.setMessageListener(listener);
        } else {
            LOG.severe("Can't set Listener, no consumer set!");
        }
        LOG.info("Amount of registered consumers " + (consumer == null ? "null" : consumer.getMessageSelector()));
    }
    ...
    public void close() {
        try {
            ...
            if (connection == null) {
                LOG.info("JMS connection is null!");
            } else {
                connection.close();
            }
            isClosed = true;
